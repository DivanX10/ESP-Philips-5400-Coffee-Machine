...
uint8_t CoffePattern[14][3]={
    ...
    {0,2,2},//Ristretto
    {1,2,3},//Americano
    {2,2,1},//Coffee With Milk
    {2,2,2},//Latte
    {3,2,2},//Milk Coffee to go
    {3,2,2},//Latte Macchiato
    {3,2,3},//Сappuccino
    {4,0,0},//Milk foam
    {5,1,0} //Hot water
};
...
void PhilipsSeries5400::coffee_start(){ // старт приготовления кофе
   coffe_route=1;
}
...
void PhilipsSeries5400::coffee_build(uint8_t type, uint8_t bean, uint8_t cups, uint16_t vol, uint16_t milk){ // старт приготовления кофе
   ...
   if(cups==2 && rec[0]!=0){ // экстрашот может быт только у черного кофе
      rec[2]=0; 
   } else {
      rec[2]=cups;
   }
              ...
              if((buffer_board[3] & 0xF0)==0x90){ // пакет 9х
                 ...
                 if( buffer_board[4]<5){ // первые пакеты не контролим
                    ...
                    if(buffer_board[3]==0x90){
                       if(sens_product!=nullptr){coffee_show(buffer_board+3);}  //  текстовая расшифровка рецепта
...
void PhilipsSeries5400::dump_config() {
    ...
    LOG_BUTTON(TAG, "Start prepare", this->start_button);
}
